...
// ===== Xác định cột category trong bảng products (ưu tiên category_code) =====
$productCategoryCol = 'category_code';
$checkCol = mysqli_query($conn, "SHOW COLUMNS FROM products LIKE 'category_code'");
if (!$checkCol || mysqli_num_rows($checkCol) === 0) {
    // fallback schema cũ
    $productCategoryCol = 'category';
}

// ===== Load categories từ DB => map code => name_vi =====
$CATEGORY_LABELS = [];
$catSql = "SELECT code, name_vi FROM categories ORDER BY sort_order ASC, name_vi ASC";
$catRes = @mysqli_query($conn, $catSql);

if ($catRes) {
    while ($c = mysqli_fetch_assoc($catRes)) {
        $CATEGORY_LABELS[$c['code']] = $c['name_vi'];
    }
}

// Fallback (phòng trường hợp bảng categories chưa tạo / query lỗi)
...
if (isset($_GET['action'])) {
    ...
    if ($action === 'edit' && isset($_GET['code'])) {
        ...
        if ($result && mysqli_num_rows($result) === 1) {
            ...
            $errors[] = "Không tìm thấy sản phẩm để sửa.";
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Lưu trong DB là "mã" (AO/QUAN/...) nhưng UI là tiếng Việt
    // (Giữ name="category" để không phải sửa validateProduct)
    $category_code = cleanInput($_POST['category'] ?? '');
    ...
    $errors = validateProduct($product_code, $product_name, $category_code, $size, $price, $quantity);

    // Check category hợp lệ theo map (giải quyết triệt để lỗi Ao/Áo)
    if (!empty($category_code) && !array_key_exists($category_code, $CATEGORY_LABELS)) {
    ...
    if (empty($errors)) {
        ...
        $category_db = mysqli_real_escape_string($conn, $category_code);
        $size_db = mysqli_real_escape_string($conn, $size);
        $price_db = mysqli_real_escape_string($conn, $price);
        $quantity_db = mysqli_real_escape_string($conn, $quantity);
        $description_db = mysqli_real_escape_string($conn, $description);

        if (!empty($_POST['old_product_code'])) {
            // ===== UPDATE =====
            $old_code = mysqli_real_escape_string($conn, $_POST['old_product_code']);

            // Nếu đổi mã sản phẩm -> check trùng
            if ($product_code_db !== $old_code) {
                if (checkProductCodeExists($conn, $product_code_db)) {
                    $errors[] = "Mã sản phẩm '$product_code_db' đã tồn tại";
                }
            }
            ...
            if (empty($errors)) {
                ...
                if ($imageFilename) {
                    ...
                    $imageSql = ", image = '" . mysqli_real_escape_string($conn, $imageFilename) . "'";
                }

                // UPDATE theo schema đang dùng (category_code hoặc category)
                $sql = "UPDATE products SET
                            product_code = '$product_code_db',
                            product_name = '$product_name_db',
                            {$productCategoryCol} = '$category_db',
                            size = '$size_db',
                            price = '$price_db',
            ...
            if (empty($errors)) {
                $imageValue = $imageFilename ? ("'" . mysqli_real_escape_string($conn, $imageFilename) . "'") : "NULL";

                $sql = "INSERT INTO products (product_code, product_name, {$productCategoryCol}, size, price, quantity, description, image)
                        VALUES ('$product_code_db', '$product_name_db', '$category_db', '$size_db', '$price_db', '$quantity_db', '$description_db', $imageValue)";
...
// ===== READ - Lấy danh sách sản phẩm (ưu tiên join categories để hiện tiếng Việt) =====
$products = null;

if ($productCategoryCol === 'category_code') {
    $sql = "
        SELECT p.*,
...
if (!$products) {
    // fallback schema cũ hoặc join lỗi
